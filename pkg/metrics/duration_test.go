...
func Test_ProxyRequestDuration(t *testing.T) {
	r := require.New(t)
	ProxyRequestDuration("GetObject", "destination", 300*time.Millisecond)
	...
	r.InDelta(0.3, h.GetSampleSum(), 0.001)
}

func Test_ProxyRequestInternalDuration(t *testing.T) {
	r := require.New(t)
	ProxyRequestInternalDuration("GetObject", "destination", 100*time.Millisecond)

	h := histogram(t, "proxy_request_internal_duration_seconds", "GetObject", "destination")
	r.NotNil(h, "series not exported")
	r.EqualValues(1, h.GetSampleCount())
	r.InDelta(0.1, h.GetSampleSum(), 0.001)

	// a request that never reached a storage still has to carry a label value
	ProxyRequestInternalDuration("GetObject", "", time.Second)
