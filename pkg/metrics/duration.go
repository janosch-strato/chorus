...
var proxyRequestDuration = promauto.NewHistogramVec(prometheus.HistogramOpts{
	...
	Help:    "Time until the storage answered a proxied request, by the storage that answered it. Excludes streaming the response body to the client.",
...
// ProxyRequestDuration records how long the storage took to answer a proxied
// request. An empty storage name means the request never reached a storage.
