...
type Service interface {
	...
	// The objects a migration has copied. The proxy serves reads of those
	// from the destination storage, see the proxy router.
	...
	DelMigratedObjs(ctx context.Context, id entity.ReplicationStatusID) error
...
// migratedObjsKey names the set of objects a migration has copied. One set
// per replication, so that deleting the replication is one key to delete.
func migratedObjsKey(id entity.ReplicationStatusID) string {
	return fmt.Sprintf("s:mig:%s:%s:%s:%s:%s", id.User, id.FromStorage, id.FromBucket, id.ToStorage, id.ToBucket)
}
...
func (s *svc) DelMigratedObjs(ctx context.Context, id entity.ReplicationStatusID) error {
